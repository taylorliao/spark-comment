...
private[master] class Master(
  ...
  private def startExecutorsOnWorkers(): Unit = {
    ...
    // 对于同一个Application，它在一个worker上只能拥有一个executor
    if (spreadOutApps) {  //尽量的打散，即一个Application尽可能多的分配到不同的节点
      // Try to spread out each app among all the workers, until it has all its cores
      //app.coresLeft表示的是该app还有cpu资源没申请到：  app.coresLeft  = 当前app申请的maxcpus - granted的cpus
      for (app <- waitingApps if app.coresLeft > 0) {  //使用FIFO的方式为等待的app分配资源/
        ...
        val assigned = new Array[Int](numUsable) // Number of cores to give on each node //候选Worker，每个Worker一个下标，是一个数组，初始化默认都是0
        // 1个APP申请了6个core, 现在有2个Worker可用。  那么： toAssign = 6，assigned = 2
   ...
   * 在选择了worker和确定了worker上得executor需要的CPU core数后,
   * Master会调用 launchExecutor(worker: WorkerInfo, exec: ExecutorInfo)向Worker发送请求，向AppClient发送executor已经添加的消息
   */
  private def allocateWorkerResourceToExecutors(
    ...
    while (coresLeft >= coresPerExecutor && worker.memoryFree >= memoryPerExecutor) {
      val exec = app.addExecutor(worker, coresPerExecutor)  //更新app里的Executor信息
      coresLeft -= coresPerExecutor
      launchExecutor(worker, exec)  //通知可用Worker去启动Executor
  ...
  private def schedule(): Unit = {
    ...
    val shuffledWorkers = Random.shuffle(workers) // Randomization helps balance drivers 把当前workers这个HashSet的顺序随机打乱
  ...
  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    ...
    worker.addExecutor(exec) //更新worker的信息，可用core数和memory数减去本次分配的executor占用的,向Worker发送启动executor的请求
    worker.actor ! LaunchExecutor(masterUrl,
      exec.application.id, exec.id, exec.application.desc, exec.cores, exec.memory)
    exec.application.driver ! ExecutorAdded(  // 向AppClient发送executor已经添加的消息
