 ...
 * ExecutorRunner会将在org.apache.spark.scheduler.cluster.SparkDeploySchedulerBackend中准备好的
 * org.apache.spark.deploy.ApplicationDescription以进程的形式启动起来。
 */
private[deploy] class ExecutorRunner(
   ...
   */
  private def fetchAndRunExecutor() { //根据ApplicationDescription携带的信息启动CoarseGrainedExecutorBackend
    try {
      // Launch the process
