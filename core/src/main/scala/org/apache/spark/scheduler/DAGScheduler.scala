 ...
 *
 * DAGScheduler在创建时会创建名字为eventProcessActor的actor
 * org.apache.spark.scheduler.DAGSchedulerEventProcessActor的作用：可以把他理解成DAGScheduler的对外的功能接口。
 * 它对外隐藏了自己内部实现的细节，也更易于理解其逻辑；也降低了维护成本，将DAGScheduler的比较复杂功能接口化.
 */
private[spark]
class DAGScheduler(
  ...
  private def getParentStages(rdd: RDD[_], jobId: Int): List[Stage] = {
    ...
    while (waitingForVisit.nonEmpty) {  //只要stack不为空，则一直处理。
      visit(waitingForVisit.pop())      //获取Stack中最后的元素，而且移除
    }
    parents.toList
  }
  ...
  // finalStage，顾名思义，就是最后的那个Stage。然后创建job，最后提交。最后的Stage包含了一组ResultTask 。有两种task
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      finalStage = newResultStage(finalRDD, partitions.size, jobId, callSite)   //创建finalStage
    } catch {
      case e: Exception =>
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      // 如果当前stage不在等待其parent stage的返回，并且不在运行的状态， 并且没有已经失败（失败会有重试机制，不会通过这里再次提交）
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        val missing = getMissingParentStages(stage).sortBy(_.id)
        logDebug("missing: " + missing)
        if (missing.isEmpty) { // 如果所有的parent stage都已经完成，那么提交该stage所包含的task
          logInfo("Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
          submitMissingTasks(stage, jobId.get)
        } else {
          for (parent <- missing) {  // 有parent stage没有完成，则递归提交它
            submitStage(parent)
          }
          waitingStages += stage
        }
  ...
  //2、序列化task的binary。Executor可以通过广播变量得到它。每个task运行的时候首先会反序列化。
  //这样在不同的executor上运行的task是隔离的，不会相互影响。
  //3、为每个需要计算的partition生成一个task：对于Shuffle类型依赖的Stage，生成ShuffleMapTask类型的task；
  //对于Result类型的Stage，生成一个ResultTask类型的task
  //4、确保Task是可以被序列化的。因为不同的cluster有不同的taskScheduler，在这里判断可以简化逻辑；
  //保证TaskSet的task都是可以序列化的
  //5、通过TaskScheduler提交TaskSet。
  //6、TaskSet就是可以做pipeline的一组完全相同的task，每个task的处理逻辑完全相同，不同的是处理数据，
  //每个task负责处理一个partition。pipeline，可以称为大数据处理的基石，只有数据进行pipeline处理，
  //才能将其放到集群中去运行。对于一个task来说，它从数据源获得逻辑，然后按照拓扑顺序，顺序执行（实际上是调用rdd的compute).
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  override def onReceive(event: DAGSchedulerEvent): Unit = event match {
    ...
    case ExecutorAdded(execId, host) =>  // TaskScheduler得到一个Executor被添加的消息
